...
const validationSchema = {
  [EAuthFormTab.LOGIN]: Yup.object({
    email: Yup.string()
      .email("Pogresan format email adrese")
    ...
    <Formik
      ...
      onSubmit={async ({ email, password, fullName }, { setSubmitting }) => {
        ...
        if (tab === EAuthFormTab.REGISTER) {
          dispatch(
            register({ email, password, fullName }, () => setSubmitting(false))
          );
        } else if (tab === EAuthFormTab.LOGIN) {
          dispatch(login(email, password, () => setSubmitting(false)));
        }
      }}
    >
        ...
        {tab === EAuthFormTab.FORGOT_PASSWORD && (
          <>
            ...
            <Typography component="div" variant="caption">
              Na ovu email adresu, ako postoji registrovan korisnik, cete dobiti
              link za promjenu sifre
            </Typography>
          </>
        )}
          ...
          {tab === EAuthFormTab.LOGIN && (
            <Grid container>
              <Grid item xs={6}>
                <Link
                  href="#"
                  onClick={() => onChangeTab(EAuthFormTab.FORGOT_PASSWORD)}
                >
                  Promjena sifre
                </Link>
              </Grid>
